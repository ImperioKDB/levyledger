...
export default function ProposalPage() {
  ...
  if (!proposal) return (
    <main className="min-h-screen bg-ink px-6 pt-12">
      ...
      <p className="font-display text-2xl font-bold text-ledger mt-8">Proposal not found</p>
    </main>
  )
    ...
    <main className="min-h-screen bg-ink">
      ...
      <section className="px-6 py-6 border-b border-rule space-y-5">
        <div>
          ...
          <p className="text-body text-sm leading-relaxed">{proposal.description}</p>
        </div>
        <div>
          <p className="font-data text-ghost text-xs mb-1">Category</p>
          <p className="font-data text-ledger text-sm">{CATEGORY_LABELS[category] || category}</p>
        </div>
        <div>
          <p className="font-data text-ghost text-xs mb-1">Recipient</p>
          <p className="font-data text-body text-xs break-all">{proposal.recipient?.toString()}</p>
        </div>
        <div>
      ...
      <section className="px-6 py-6 pb-28">
        ...
        <div className="space-y-3">
          <div className="flex justify-between">
            ...
            <span className="font-data text-xs text-ledger">{fmtTimestamp(proposal.createdAt)}</span>
          </div>
          <div className="flex justify-between">
            <span className="font-data text-xs text-ghost">
              {status === 'expired' ? 'Expired' : 'Expires'}
            </span>
            <span className="font-data text-xs text-ledger">{fmtTimestamp(proposal.expiresAt)}</span>
          </div>
        </div>
      </section>
    </main>
